...
bool Root::ManualMove::handleEvent ( GeneratedEventBase* event ) {
  ...
  case EventType::AtDestinationEvent:
  case EventType::AutoNextPassEvent:
  case EventType::AutoReturnToStartPosition:
  case EventType::EStopEvent:
  case EventType::MoveToEvent:
  case EventType::NextPassEvent:
  case EventType::ResetEvent:
  case EventType::ReturnToStartEvent:
  case EventType::SetAutoMode:
  case EventType::StartEvent:
  case EventType::StartMovingEvent:
  case EventType::StopEvent:
  case EventType::WaitForUserEvent:
    handled = true;
    break;
  default:
    handled = false;
  ...
  if (!handled) {
    ...
    switch ( event->get_type() ) {
    case EventType::ResumeLastStateEvent:
      ...
      _root->log("\033[31mSTATE TRANSITION: ManualMove->ResumeLastState\033[0m");
        ...
        _root->MOVEMENT_OBJ.setDeepHistory();
  ...
  if (!handled) {
    ...
    case EventType::ResetEvent:
      ...
      _root->log("\033[31mSTATE TRANSITION: EStop->ResumeLastMode\033[0m");
        ...
        _root->MOVEMENT_OBJ.setShallowHistory();
          // make sure nothing else handles this event
        handled = true;
        }
      break;
